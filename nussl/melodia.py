...
class Melodia(separation_base.SeparationBase):
    ...
    def extract_melody(self):
        ...
        data = vamp.collect(self.audio_signal.audio_data, self.sample_rate, "mtg-melodia:melodia", parameters=params)
    ...
    def create_melody_signal(self, num_overtones):
        ...
        for time, frequency in zip(self.timestamps, self.melody):
            ...
            if num_samples > 0:
                num_transition_samples = float(min(np.round(transition_length * sample_rate), num_samples))
    ...
    def run(self):
        ...
        Returns:
            foreground (AudioSignal): An AudioSignal object with melodic foreground in foreground.audio_data
